...
import 'package:open_weather_example_flutter/src/features/weather/data/apiException.dart';
import 'package:open_weather_example_flutter/src/features/weather/data/weatherRepository.dart';
...
void main() {
  test('repository with mocked http client, success', () async {
    ...
    final api = WeatherApiServiceClass('apiKey');
    final weatherRepository =
        HttpWeatherRepository(api: api, client: mockHttpClient);
    when(() => mockHttpClient.get(api.weather('Pattoki'))).thenAnswer(
        (_) => Future.value(http.Response(encodedWeatherJsonResponse, 200)));
    final weather = await weatherRepository.getWeather(city: 'Pattoki');
    // expectations
  ...
  test('repository with mocked http client, failure', () async {
    ...
    final weatherRepository =
        HttpWeatherRepository(api: api, client: mockHttpClient);
    when(() => mockHttpClient.get(api.weather('Pattoki')))
        .thenAnswer((_) => Future.value(http.Response('{}', 401)));
    // expectations
    expect(() => weatherRepository.getWeather(city: 'Pattoki'),
